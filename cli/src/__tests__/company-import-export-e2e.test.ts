...
function createBasePaperclipEnv(options: TestPaperclipEnv) {
  ...
  // The server/CLI also load the repo-root .env by default. Pin the intended
  // test deployment mode so a developer's local .env cannot override the
  // explicit test config and make local_trusted API setup require auth.
